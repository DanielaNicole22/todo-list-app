...
Spec Kit is configured for Codex with PowerShell scripts and skill-based commands. The React application is still an early scaffold: `App.tsx` renders an empty container and the files under `src/components/` are placeholders.
...
5. Connect the components through `App.tsx`.
